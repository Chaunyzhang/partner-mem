...
describe("Partner-Mem OpenClaw tools", () => {
  ...
  it("binds recall to trusted OpenClaw context instead of model-supplied identity", async () => {
    const runtime = createTempRuntime();
    try {
      runtime.ingest.ingestTurn({
        agent_id: "agent-1",
  ...
  it("defaults recall to current session and only crosses sessions with agent_memory scope", async () => {
    ...
    try {
      ...
      const currentSession = await recallTool.execute("call-current-session", {
        query: "scoped proof",
        limit: 5
      });
      const agentMemory = await recallTool.execute("call-agent-memory", {
        query: "scoped proof",
        scope: "agent_memory",
        limit: 5
      });
